...
# Apply K-Means clustering with the optimized number of clusters (k=4)
...
for icoord, dcoord, color in zip(dendro['icoord'], dendro['dcoord'], dendro['color_list']):
    plt.plot(icoord, dcoord, color=color, lw=3)  
for spine in ax.spines.values():
    spine.set_edgecolor('black')
    spine.set_linewidth(2)
...
# SHAP Summary Plot for Different Clusters
def plot_shap_for_cluster(cluster_label, shap_values, cluster_data, feature_names):
    plt.figure(figsize=(12, 10))

    shap_values_for_cluster = np.abs(shap_values[cluster_label - 1]).mean(axis=0) 
...
# Distance to centroid Violin plot
sns.set(style="whitegrid")
plt.figure(figsize=(12, 10))
...
plt.xticks(fontsize=24)
plt.yticks(fontsize=24)
plt.tick_params(axis='x', which='both', direction='in', length=6, width=1.5, colors='black', top=False) 
plt.tick_params(axis='y', which='both', direction='in', length=6, width=1.5, colors='black', right=False)
plt.tick_params(axis='x', which='major', length=8, width=1.5, direction='in') 
plt.tick_params(axis='y', which='major', length=8, width=1.5, direction='in') 
ax = plt.gca()
...
# Clusterwise Pair plot
sns.set(style="white", palette="muted")

plt.figure(figsize=(22, 16))
g = sns.pairplot(features_with_clusters, hue='Cluster', palette='Set2', 
                 markers=['o', 'o', 'o', 'o'], diag_kind='kde',  
                 plot_kws={'alpha': 0.5, 's': 120, 'edgecolor': 'k'}, 
                 diag_kws={'shade': True}) 
g.fig.tight_layout()
