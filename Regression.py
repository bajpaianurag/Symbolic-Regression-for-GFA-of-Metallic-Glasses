...
# Load dataset
data = pd.read_csv('input_regression.csv')
...
# Load dataset
data = pd.read_csv('regression_pred.csv')
X_pred = data.values
...
for name, (model, search_space) in regressor_models.items():
    ...
    results[name] = {'y2_train_pred': y2_train_pred, 'y2_test_pred': y2_test_pred, 'Tx_predictions': Tx_pred, 'mse': mse, 'r2': r2, 'mae': mae}    
...
for name, (model, search_space) in regressor_models.items():
    ...
    Tl_pred = model1.predict(X_pred)
    
    mse = mean_squared_error(y3_test, y3_test_pred)
    r2 = r2_score(y3_test, y3_test_pred)
    mae = mean_absolute_error(y3_test, y3_test_pred)
    
    results[name] = {'y3_train_pred': y3_train_pred, 'y3_test_pred': y3_test_pred, 'Tl_predictions': Tl_pred, 'mse': mse, 'r2': r2, 'mae': mae}
...
df = pd.DataFrame(Tg_pred)

# Saving to Excel
df.to_excel('Tg_pred.xlsx', index=False, header=False)
...
# Creating a DataFrame
df = pd.DataFrame(Tx_pred)

# Saving to Excel
df.to_excel('Tx_pred.xlsx', index=False, header=False)
...
# Creating a DataFrame
df = pd.DataFrame(Tl_pred)

# Saving to Excel
df.to_excel('Tl_pred.xlsx', index=False, header=False)
